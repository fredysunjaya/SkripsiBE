...
class UserFaceRegister(APIView):
    def post(self, request):
        ...
        value = {"face_vector": embeddings[0]["embedding"]}

        serializer = UserSerializer(user, data=value, partial=True)
...
class UserFaceLogin(APIView):
    def post(self, request):
        ...
        try:
            ...
            embeddings = DeepFace.represent(img_path=temp_path, model_name="Facenet512")
            input_vector = embeddings[0]["embedding"]
            input_vector_str = (
                "[" + ",".join(map(str, input_vector)) + "]"
            )  # format for pgvector
        ...
        """,
            ...
            if not nearest_user:
                return Response(
                    {"error": "No users found"}, status=status.HTTP_401_UNAUTHORIZED
                )
        ...
        if distance > threshold:
            return Response(
                {"error": "Face does not match"}, status=status.HTTP_401_UNAUTHORIZED
            )
